...
## Тесты
Автотестов в проекте нет (сознательно). Проверка ручная:
...
## Состояние и планы
Планы и состояние — во внутренней вике `_local_docs/internal/roadmap.md` (не в git):
что сделано, до запуска, домен+SEO, инфра, тех-долг. Прод развёрнут (raw-порт :8092),
автодеплой настроен (`dev → PR → auto-merge → deploy`).
